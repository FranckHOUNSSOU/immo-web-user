...
export default function NotificationsPage() {
  ...
  if (!isLoggedIn) return (
    <div className="min-h-dvh bg-app-bg flex flex-col items-center justify-center py-20 text-center px-6">
      ...
      <div className="max-w-3xl mx-auto px-4 md:px-6">
          ...
          <div className="flex gap-2">
            {[['toutes', 'Toutes'], ['non_lues', `Non lues${unread > 0 ? ` (${unread})` : ''}`]] .map(([k, l]) => (
              <button
                ...
                className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all ${filter === k ? 'bg-primary text-white' : 'bg-surface-g text-text-grey hover:bg-gray-100'}`}
              >
                {l}
              </button>
            ))}
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <div className="w-20 h-20 bg-surface-g rounded-full flex items-center justify-center mb-4">
            ...
            {displayed.map(n => {
              ...
              return (
                <div
                  ...
                  className={`bg-white rounded-2xl p-4 flex items-start gap-4 transition-all cursor-pointer hover:shadow-sm ${n.lue ? '' : 'ring-1 ring-primary/10'}`}
                >
                  {/* Icon */}
                  <div
